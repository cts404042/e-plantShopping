    ...
    <div className="product-list"> {/* Container for the list of plant cards */}
      {category.plants.map((plant, plantIndex) => (
        <div className="product-card" key={plantIndex}> {/* Unique key for each plant card */}
          ...
          <div className="product-cost">${parseFloat(plant.cost.substring(1))}</div> {/* Display plant cost */}
          
          <button
            className={`product-button ${
    cart.some(item => item.name === plant.name) ? "added-to-cart" : ""
  }`}
            onClick={() => handleAddToCart(plant)} disabled={cart.some(item => item.id === plant.name)} // Handle adding plant to cart
          >
